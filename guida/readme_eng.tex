...
\begin{itemize}
	...
	\item \texttt{\textbackslash usepackage[comfortaa]\{PadovaTheme\}} allows to use the font style Comfortaa instead of the default one (requires the package \texttt{comfortaa} and the intallation of the font family Comfortaa \cite{web:comfortaa}).
\end{itemize}
...
\begin{itemize}
	...
	\item [-]\texttt{\textbackslash correlatore[}\textit{assistant label}\texttt{]\{}\textit{co-supervisor}\texttt{\}} For the eventual co-supervisor of your thesis. The optional argument \textit{assistant label} provides a different label with the respect to ``Correlatore''.
	\item [-]\comando{altricorrelatori}{others co-supervisors} To insert other eventual co-supervisors. If more than one, each name must be delimited by the command``\texttt{\textbackslash\textbackslash}''.
